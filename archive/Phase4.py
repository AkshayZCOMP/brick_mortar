...
class Params:
    ...
    #ply staggering parameters
    n_plies: int = 2
    stagger: bool = True
    stagger_offsetx: float = L_fiber / 2  #mm
    stagger_offsety: float = row_width / 2  #mm
#======================================================================================
#staggering helper
...
def dof_index(ply: int, row: int, node: int, n_nodes: int) -> int:
    # global DOF index for 1 DOF per node per row
    return int(ply * row * n_nodes + row * n_nodes + node)
#======================================================================================
#element assembly
#=====================================================================================
...
#build phase 1 model
#=====================================================================================
def assemble_ply(params: Params, x:np.ndarray):
    n_nodes = len(x)
...
#ASSEMBLE PLYS
#=====================================================================================
def assemble_plys(params: Params, x:np.ndarray):
    K1, f1, elems1 = assemble_ply(params, x)  # 
    K2, f2, elems2 = assemble_ply(params, x)  # 
    
    n_nodes = len(x)
    n_dofs = params.n_rows * n_nodes *params.n_plies
    ...
    elems = [] 
    evenoffset, oddoffset = staggering_coords(params)
    if params.stagger == False:
        for row in range(params.n_rows):
            for i in range(n_nodes-1):
                a = ply1.dof_index(row, i, n_nodes)
                b = ply1.dof_index(row, i, n_nodes)
                add_2node_spring(K, a, b, ksh)
                elems.append({"etype": "shear", "i": i, "a": a, "b": b,  "k": ksh})
    elems.append({"etype": "shear", "i": i, "a": a, "b": b,  "k": ksh})
...
def main():
    ...
    K, f ,elems= assemble_ply(params, x)
